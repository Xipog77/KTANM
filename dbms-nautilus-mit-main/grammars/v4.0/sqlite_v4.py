# sqlite_v4.py — Attack surface grammar for Nautilus SQLite fuzzing
# Version: v4.0 (zero-day hunting on SQLite 3.53.0)
#
# DESIGN: Extends v3.4 structural primitives with:
...
#   - New SQLite 3.44+ functions (parseuri, unhex, unistr, math, median/percentile)
...
# Schema mutation: ALTER after CREATE (exercises schema-change code paths)
ctx.rule("Sql-Stmt",
    "{Schema-Setup};\n{Alter-Table-Stmt}",
    weight=1.0)

# Schema lifecycle: CREATE + populate + DROP (exercises cleanup code)
ctx.rule("Sql-Stmt",
    "{Schema-Setup};\n{Insert-Stmt};\n{Drop-Stmt}",
    weight=0.5)

# Standalone PRAGMA (exercises pragma code paths directly)
...
# Attach + cross-database query
ctx.rule("Sql-Stmt",
    "{Attach-Stmt};\n{Stress-Query}",
    weight=0.5)

# Maintenance operations
ctx.rule("Sql-Stmt", "{Vacuum-Stmt}", weight=0.3)
ctx.rule("Sql-Stmt", "{Reindex-Stmt}", weight=0.3)
...
ctx.rule("Sql-Inner-Stmt", "{Pragma-Stmt}")
ctx.rule("Sql-Inner-Stmt", "{Alter-Table-Stmt}")
...
# Asc-Desc inlined into Ordering-Term rules (not a standalone non-terminal)
...
# v4.0: CTE limits for recursive CTEs (controls recursion depth)
ctx.rule("Cte-Limit", "100")
ctx.rule("Cte-Limit", "1000")
ctx.rule("Cte-Limit", "10000")
ctx.rule("Cte-Limit", "100000")
ctx.rule("Cte-Limit", "1000000")
...
# Schema with trigger (exercises trigger code paths)
ctx.rule("Schema-Setup",
    "CREATE TABLE IF NOT EXISTS p({Col-Def-List});\n"
    "{Create-Trigger-Stmt}",
    weight=1.0)

# Schema with table + view + index (complex topology)
ctx.rule("Schema-Setup",
    "CREATE TABLE IF NOT EXISTS p({Col-Def-List});\n"
    "CREATE VIEW IF NOT EXISTS v1 AS {Select-Stmt};\n"
    "CREATE INDEX IF NOT EXISTS idx1 ON p({Col-Name})",
...
# Standalone DDL alternatives (wires Layer 1 DDL into Layer 2)
ctx.rule("Schema-Setup", "{Create-Table-Stmt}", weight=0.5)
ctx.rule("Schema-Setup", "{Create-View-Stmt}", weight=0.5)
ctx.rule("Schema-Setup",
    "{Create-Table-Stmt};\n{Create-Index-Stmt}",
    weight=0.5)
ctx.rule("Schema-Setup", "{Create-Virtual-Table-Stmt}", weight=0.3)
...
# Recursive CTE + aggregation (compositional — fuzzer must compose the function)
ctx.rule("Stress-Query",
    "WITH RECURSIVE cnt(x) AS (VALUES(1) UNION ALL SELECT x+1 FROM cnt WHERE x < {Cte-Limit}) "
    "SELECT {Func-Call} FROM cnt",
    weight=3.0)

# Recursive CTE + SELECT expression (fully compositional)
ctx.rule("Stress-Query",
    "WITH RECURSIVE cnt(x) AS (VALUES(1) UNION ALL SELECT x+1 FROM cnt WHERE x < {Cte-Limit}) "
    "SELECT {Result-Col-List} FROM cnt",
    weight=2.5)
...
# parseuri stress (compositional: Uri-Literal provides domain-specific inputs)
ctx.rule("Stress-Query",
    "SELECT parseuri({Uri-Literal}, {Boundary-Int}) FROM {Table-Name}",
    weight=2.5)

# unistr stress (compositional: Unicode-Literal provides escape sequences)
ctx.rule("Stress-Query",
    "SELECT unistr({Unicode-Literal}) FROM {Table-Name}",
    weight=2.5)
...
# ============================================================
ctx.rule("Validation-Op", "{Pragma-Stmt}", weight=3.0)
ctx.rule("Validation-Op", "{Analyze-Stmt}", weight=1.5)
ctx.rule("Validation-Op", "EXPLAIN QUERY PLAN {Select-Stmt}", weight=1.5)
ctx.rule("Validation-Op", "{Vacuum-Stmt}", weight=0.5)
ctx.rule("Validation-Op", "{Reindex-Stmt}", weight=0.5)
