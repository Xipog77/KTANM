# sqlite_v4_debug.py — Debug-build grammar for Nautilus SQLite fuzzing
# Version: v4.0c (re-adds debug functions + additional PRAGMA/EXPLAIN stress)
#
...
# SQLITE_DEBUG functions (re-added for debug harness campaigns)
ctx.rule("Func-Call", "fpdecode({Expr}, {Expr}, {Expr})", weight=1.5)
ctx.rule("Func-Call", "fpdecode({Boundary-Float}, {Boundary-Int}, {Boundary-Int})", weight=2.0)
ctx.rule("Func-Call", "parseuri({Expr}, {Expr})", weight=2.5)
ctx.rule("Func-Call", "parseuri({Str-Literal}, {Boundary-Int})", weight=2.0)
ctx.rule("Func-Call", "parseuri({Str-Literal}, {Boundary-Int}, {Str-Literal})", weight=1.5)
...
# v4.0c: Debug-specific stress patterns
# parseuri with various URI forms
ctx.rule("Stress-Query",
    "SELECT parseuri({Str-Literal}, {Boundary-Int})",
    weight=3.0)

# EXPLAIN of complex queries (exercises VdbeDisplay/VdbeList in debug builds)
ctx.rule("Stress-Query",
    "EXPLAIN {Select-Stmt}",
    weight=2.5)

# fpdecode with various float/int combos
ctx.rule("Stress-Query",
    "SELECT fpdecode({Boundary-Float}, {Boundary-Int}, {Boundary-Int})",
    weight=2.0)
...
ctx.rule("Validation-Op", "EXPLAIN {Select-Stmt}", weight=2.0)
...
# SQLITE_DEBUG boundary calls (re-added for debug campaigns)
ctx.rule("Boundary-Func-Call",
    "fpdecode({Boundary-Float}, {Boundary-Int}, {Boundary-Int})", weight=2.5)
ctx.rule("Boundary-Func-Call",
    "parseuri({Str-Literal}, {Boundary-Int})", weight=2.5)
ctx.rule("Boundary-Func-Call",
