...
CREATE TABLE IF NOT EXISTS p(a VARCHAR(255) CHECK (row_number() FILTER (WHERE RAISE(IGNORE)) OVER () + count() FILTER (WHERE CURRENT_TIMESTAMP IS DISTINCT FROM CURRENT_TIMESTAMP)));
CREATE TABLE IF NOT EXISTS q(c1 NUMERIC UNIQUE);
